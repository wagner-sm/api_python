...
class SetlistFMScraperPlaywright:
    def __init__(self, username, headless=True):
        ...
        # Dicionário de festivais: nome -> {local, cidade}
        self.festival_cities = {
            'Bangers':                     {'local': 'Memorial da América Latina',          'cidade': 'São Paulo'},
            'Março Maldito':               {'local': 'Carioca Club',          'cidade': 'São Paulo'},
            'Summer Breeze':               {'local': 'Memorial da América Latina',      'cidade': 'São Paulo'},
            'Rise CWB Metal 2019':         {'local': 'Jokers Pub',      'cidade': 'Curitiba'},
            'Solid Rock Curitiba 2017':    {'local': 'Pedreira Paulo Leminski',          'cidade': 'Curitiba'},
            'Solid Rock Curitiba 2018':    {'local': 'Pedreira Paulo Leminski',          'cidade': 'Curitiba'},
            'Skol Rock 1997':              {'local': 'Marumby Expo Center',      'cidade': 'Curitiba'},
            'Skol Rock 1998 (Curitiba)':   {'local': 'Pedreira Paulo Leminski',      'cidade': 'Curitiba'},
            'Armageddon Metal Fest 2014':  {'local': 'Expoville Convention Center',       'cidade': 'Joinville'},
            'Armageddon Metal Fest 2019':  {'local': 'Expoville Convention Center',       'cidade': 'Joinville'},
            'Armageddon Metal Fest 2023':  {'local': 'Luna Live',       'cidade': 'Joinville'},
            'Dead Shall Rise 2011':        {'local': 'Hangar Bar',       'cidade': 'Curitiba'},
            'Rock Ao Vivo - Curitiba 2019':{'local': 'Pedreira Paulo Leminski',      'cidade': 'Curitiba'},
            'Setembro Negro Festival 2022':{'local': 'Carioca Club',          'cidade': 'São Paulo'},
            'Setembro Negro Festival 2023':{'local': 'Carioca Club',          'cidade': 'São Paulo'},
            'Setembro Negro Festival 2025':{'local': 'VIP Station',          'cidade': 'São Paulo'},
            'Crossroads Festival 2023':    {'local': 'Pedreira Paulo Leminski',         'cidade': 'Curitiba'},
            'Angra Fest Curitiba 2023':    {'local': 'White Hall Jockey Eventos',         'cidade': 'Curitiba'},
            'Genocide Fest 2019':          {'local': 'Jokers Pub','cidade': 'Curitiba'},
            'Overload Beer Fest 2019':     {'local': 'Carioca Club',      'cidade': 'São Paulo'},
            'Liberation Festival 2017':    {'local': 'Espaço das Américas',          'cidade': 'São Paulo'},
            'Liberation Festival 2018':    {'local': 'Audio',          'cidade': 'São Paulo'},
            'Monsters of Rock Brazil 2015':{'local': 'Arena Anhembi',      'cidade': 'São Paulo'},
            'Zoombie':                     {'local': 'Fazenda Evaristo',     'cidade': 'Rio Negrinho'},
            "Live 'N' Louder":             {'local': 'Arena Skol Anhembi',          'cidade': 'São Paulo'},
            'Guaru':                       {'local': 'King Club',       'cidade': 'Guarulhos'},
            'Visions':                     {'local': 'Hangar 110',          'cidade': 'São Paulo'},
            'Brazilian Metal Forces 2026': {'local': 'Hard Rock Café','cidade': 'Curitiba'},
        }
    ...
    def identify_festival(self, local_text):
        if not local_text:
            return None
        
        for festival_name, info in self.festival_cities.items():
            if festival_name.lower() in local_text.lower():
                return {'festival': festival_name, 'local': info['local'], 'cidade': info['cidade']}
        ...
        try:
            ...
            if subline:
                ...
                if location_span:
                    ...
                    festival_info = self.identify_festival(location_text)
                    if festival_info:
                        show_data['Local'] = festival_info['local']
                        show_data['Festival'] = location_text
                        show_data['Cidade'] = festival_info['cidade']
    ...
    def scrape_current_page(self):
        try:
            ...
            # Filtra apenas shows attended (href=/setlist/) vs upcoming (href=/show/)
            attended_shows_html = []
            for element in show_elements:
                try:
                    html = element.inner_html()
                    if 'href="../setlist/' in html:
            ...
            for html in attended_shows_html:
                ...
                if show_data and show_data['Artista'] != 'N/A' and show_data['Data'] != 'N/A':
    ...
    def scrape_all_shows(self):
        ...
        try:
            ...
            while consecutive_empty_pages < max_empty_pages and page_number <= max_pages:
                ...
                if page_shows:
                    # Verifica duplicatas
                    existing_shows = {(s['Artista'], s['Data'], s['Local'], s['Festival']) for s in self.all_shows}
                    new_shows = [s for s in page_shows 
                               if (s['Artista'], s['Data'], s['Local'], s['Festival']) not in existing_shows]
    ...
    def generate_excel_base64(self):
        ...
        # Remove duplicatas
        df = df.drop_duplicates(subset=['Artista', 'Data', 'Local', 'Festival'])
